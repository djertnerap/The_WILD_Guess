...
def main():
    ...
    for file in file_names:             #create metadata, algo and eval concatenated dataframes from the files in the log directory
        if file.startswith('split') & (not args.eval_only):
            ...
            if not (args.eval_only & file.endswith('train_eval.csv')):
    ...
    if (len(metadata_df) != 0) & (not args.eval_only):       # create meatadata graphs only if the files are present
    ...
    (str(avg_train_acc) if not args.eval_only  else '') + '\t' + \
    str(avg_id_val_acc) + '\t' + \
    str(avg_ood_val_acc) + '\t' + \
    str(avg_id_test_acc) + '\t' + \
    str(avg_ood_test_acc) + '\t' + \
    (str(avg_rg_train_acc) if not args.eval_only  else '') + '\t' + \
    str(avg_rg_id_val_acc) + '\t' + \
    str(avg_rg_ood_val_acc) + '\t' + \
    str(avg_rg_id_test_acc) + '\t' + \
    str(avg_rg_ood_test_acc) + '\t' + \
    (str(wrst_rg_train_acc) if not args.eval_only  else '') + '\t' + \
